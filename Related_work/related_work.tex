...
In real life, we usually encounter a type of problems in which its output (or output sequence) is determined by an variable-length sequence of input data points. 
...
In layman term, given an output \(O_{t1}\) and a trainable parameter \(W\) at time \(t_2\) which denoted as \(W^{(t_2)}\),  exploding (vanishing) gradient can be understand as the norm \(L2\) of the gradient \(\frac{\partial O}{\partial W^{(t_2)}}\) get exponentially bigger (smaller) with respect to the number of time steps \((t_1-t_2)\)\cite{Bengio1994}.  

For exploding gradient, an ad-hoc technique called gradient clipping\cite{gradient-clip} was introduced and proven to be effective. 
...
\begin{align}
  	G &:= \frac{Gm}{\norm{G}} &
\end{align}
To mitigate the harm of vanishing gradient, Long Short Term Memory unit (LSTM)\cite{originLSTM} unit was invented. 
...
\begin{align}
	w_t &= \sigma(W^{(w)}i_t + U^{(w)}h_{t-1} + b^{(w)}) \label{eq:input-gate}&\\ 
  	f_t &= \sigma(W^{(f)}i_t + U^{(f)}h_{t-1} + b^{(f)}) \label{eq:forget-gate}&\\ 
  	o_t &= \sigma(W^{(o)}i_t + U^{(o)}h_{t-1} + b^{(o)}) \label{eq:output-gate}&\\ 
  	u_t &= tanh(W^{(u)}i_t + U^{(u)}h_{t-1} + b^{(u)}) \label{eq:update-gate}&\\ 
...
The output (sequence) of the LSTM unit is analogous to that of a Vanilla Recurrent unit.
...
Other variation on LSTM include adding peephole to three gates of LSTM\cite{peephole} which resulting the following modification:
\begin{align}
...
Another variation of LSTM unit is GRU unit.
