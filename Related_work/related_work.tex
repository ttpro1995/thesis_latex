...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{figure/lstm}
	\caption{A diagram of LSTM unit (back connections). By adding blue connections, we get peephole LSTM unit\footnote{Source: https://devblogs.nvidia.com/parallelforall/deep-learning-nutshell-sequence-learning/}.}
	\label{fig:lstm}
\end{figure}
...
In practice, LSTM and GRU performance are comparable and depend on tasks at hand\cite{understand-lstm}\cite{evaluate-GRU}\cite{lstm-search}. For that reason, in experiments, we should try both models. 

\subsubsection{Staking Multiple Layers of RNNs}
