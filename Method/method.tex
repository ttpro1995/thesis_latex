...
Let denote \(x_i \in \mathbb{R}^k\) as a \(k\)-dimension vector presentation of word-\(i\)th in a sentence.
...
Given \(F\) is the set of filters of the convolution layer and the filter \(v \in F\) which has window of size \(l\) and set of parameters \(\theta^{(v)} = \{ W^{(v)}, b^{(v)} | W^{(v)} \in \mathbb{R}^{l \times d}, b^{(v)} \in \mathbb{R}\}\), filter \({v}\) is applied on a sequence of word-\(i\)th to word-\((i+l-1)\)th through the following equation:
\begin{align}
...
For the case of multichannel input, given that the sentence is presented by a set of input channels \(Z\).
...
These conditions entail \({\forall u \in Z,  c^{(u)} \in \mathbb{R}^n}\)~\cite{conv-arith}.
...
Given an input vector \({x}\), the calculation steps inside the leaf module can be expressed as follow:
\begin{align}
...
Given sentence \({s}\) of length \({n}\) and its parse tree, originally, Tree-LSTM composes the output vectors by first applying its leaf module on each word vectors then applying its composition function at each node of the parse tree in a bottom-up manner.
