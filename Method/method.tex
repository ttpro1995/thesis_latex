...
\begin{description}
\item[\deschyperlink{sec:VTtree}{Tag Embeddings Tree-LSTMs}] The first approach is mainly based on the success of Tag Embeddings Recursive Neural Networks\cite{tag-embedding-rnn}.
...
We preprocess SST as described in Sec.\ref{sec:VTtreeConstituency} and Sec.\ref{sec:VTtreeDependency}. We use default train/dev/test split after removing neutral sentences (6920/872/1821). We only remove neutral sentence but keep neutral text span of positive or negative sentences. We fine tune our model on development dataset using grid search.
...
Each convolution layer contains n filter. Each filter has dimension $wd$, with \(d\) is word vector dimensions and \(w\) is word-level kernel size. 
We may have one or more kernel size, with are treated as models' hyper-parameter. 
...
Thus, n filter produces a $l \times n$ matrix with $l$ is sentence length. 
We use ReLU activation function \cite{hahnloser2000digital}. Fig \ref{fig:convlayer} illustrates convolution process. 
...
\subsubsection{Training Glove embedding on Amazon reviews dataset}\label{sec:gloveamazone}
...
This method allowing multiple parallel embedding layers in our model. 
