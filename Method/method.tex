...
We use Word Language model implementation \footnote{PyTorch Example \url{https://github.com/pytorch/examples/tree/master/word_language_model}} to pre-train CNN layer and LSTM layer. We modified the code to add convolution layer as describe in Section \ref{sec:conv1c} and Section \ref{sec:conv2c}. We keep the encoder (embedding layer \ref{sec:embedding}) and decoder (word classifier). 
...
\subsubsection{Training method and hyper-parameter}
We initialize word vector with Glove Common Crawl and Glove Amazon Reviews pre-trained word vector for CNN two channel. We set embedding vector size of 300, memory size of 168 to match with our CNN-LSTM model. We keep everything else as default setting. 

After training, we save parameters of CNN and LSTM layer, then use to initialize corresponding layer parameters for CNN-LSTM \ref{sec:CNNtree} for both cased, one channel CNN and two channel CNN.
